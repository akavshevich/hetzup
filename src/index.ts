...
import { create_nginx_config, handle_ssl, read_hetzner_config, read_server_config, update_hetzner_config, update_server_config } from './lib/configs';
...
async function test()
{
	try 
	{
		...
		// await configure_ports(selected_server);
		create_nginx_config(selected_server, {ssh: {local: 1338, remote: 22}, domains: ['heedyou.com', 'timeandmood.com']});
		// handle_ssl('heedyou.com');
	}
	catch (error)
	{
		console.log(error);
